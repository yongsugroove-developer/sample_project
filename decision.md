...
|---|---|---|---|---|
| FE-01 | proposed | 추천 믹스 비율 계산 | `Raise/Call/Fold` 합계가 항상 100%가 되도록 계산식을 보정한다. | 보정 방식(정규화 vs fold 재계산) 선택 |
| FE-02 | proposed | 카드 슬롯 DOM 구조 | 버튼 중첩 제거를 위해 슬롯 컨테이너 + 선택 버튼 + 삭제 버튼 구조로 변경한다. | 마크업 변경 범위 승인 |
| FE-03 | proposed | 고비용 계산 UX | 시뮬레이션을 청크 처리하고 계산 중 재클릭을 차단한다. | 진행 상태 표시 수준(간단 문구 vs 진행률) |
